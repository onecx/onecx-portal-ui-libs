...
import { OnecxThemeSvcContainer, StartedOnecxThemeSvcContainer } from '../containers/svc/onecx-theme-svc'
import axios from 'axios'
describe('Default workspace-svc Testcontainer', () => {
  let pgContainer: StartedOnecxPostgresContainer
  let kcContainer: StartedOnecxKeycloakContainer
  let themeSvcContainer: StartedOnecxThemeSvcContainer
  ...
  beforeAll(async () => {
    ...
    themeSvcContainer = await new OnecxThemeSvcContainer(onecxSvcImages.ONECX_THEME_SVC, pgContainer, kcContainer)
      .withNetwork(network)
      .start()
  })
