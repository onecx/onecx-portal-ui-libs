...
import { OnecxPermissionSvcContainer, StartedOnecxPermissionSvcContainer } from '../containers/svc/onecx-permission-svc'
...
describe('Default workspace-svc Testcontainer', () => {
  ...
  let permissionSvcContainer: StartedOnecxPermissionSvcContainer
  ...
  beforeAll(async () => {
    ...
    tenantSvcContainer = await new OnecxTenantSvcContainer(onecxSvcImages.ONECX_TENANT_SVC, pgContainer, kcContainer)
      .withNetwork(network)
      .start()
    permissionSvcContainer = await new OnecxPermissionSvcContainer(
