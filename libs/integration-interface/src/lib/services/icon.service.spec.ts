...
interface MockTopic<T> {
  publish(value: T): Promise<void>
  pipe(...operations: any[]): any
  destroy(): void
}

const createMockTopic = <T>(): MockTopic<T> => {
  ...
  return {
    ...
    pipe: (...operations: any[]) => (subject as any).pipe(...operations),
    destroy: jest.fn()
  }
}
...
jest.mock('../topics/icons/v1/icon.topic', () => {
  ...
  return {
    ...actual,
    IconTopic: jest.fn().mockImplementation(() => createMockTopic()),
  }
})
...
describe('IconService', () => {
  ...
  describe('requestIcon', () => {
    it('should return normalized class and publish IconRequested on first request', () => {
      ...
      const topic = (iconService.iconTopic as unknown) as MockTopic<any>
