 ...
 */
type ObjKeys<T> = T extends (...args: unknown[]) => unknown ? never : T extends object ? keyof T & string : never

/**
 * Distributively indexes into object members of a union type.
 * For each object member of `T` that has key `K`, returns `T[K]`.
 */
type ObjIndex<T, K extends string> = T extends object ? (K extends keyof T ? T[K] : never) : never
