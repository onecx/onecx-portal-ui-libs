...
export function attachPrimeReactScoper({
  ...
  const writeChunk = (styleId: string, scopedCss: string) => {
    ...
    console.log(
      `[writeChunk] writing scoped CSS for styleId=${styleId}`,
      styleTag,
      scopedCss,
    );
  ...
  const upsertScopedBlock = (styleId: string, rawCss: string) => {
    ...
    if (blockFurtherUpdatesForCapturedIds && capturedIds.has(styleId)) {
      console.log(
        '[upsertScopedBlock] skip: global lock; ignoring further updates',
        styleId,
      );
      return;
    }
    ...
    if (
      enableThemeCaptureOnce &&
      themeStyleId &&
      styleId === themeStyleId &&
      capturedIds.has(styleId)
    ) {
      console.log(
        '[upsertScopedBlock] skip: theme captured once; ignoring further updates',
        styleId,
      );
      return;
    }
          ...
          // Theme style capture path
            ...
            (node.parentElement.hasAttribute('data-app-styles') ||
              (productName &&
                ...
                node.parentElement.id.startsWith(
                  `${productName}|${productName}-ui`,
                )))
          ) {
            const attrName = node.parentElement.hasAttribute('data-app-styles')
              ? 'data-app-styles'
              : 'id';
            processStyleElement(
              node.parentElement as HTMLStyleElement,
              attrName,
            );
          }
        });
