...
export const lazyLoadingMergeReducer = (state: any, rehydratedState: any, action: any) => {
  ...
  const keysToRehydrate = Object.keys(rehydratedState).filter(
    (key) => state[key],
  );
  if (keysToRehydrate.length) {
    const stateToRehydrate = Object.keys(rehydratedState).reduce(
