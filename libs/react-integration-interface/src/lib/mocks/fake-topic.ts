...
import { Subject, Observable, Observer, Subscription, UnaryFunction, ReplaySubject } from 'rxjs'
...
export class FakeTopic<T> extends Topic<T> {
  ...
  subscribe(
    ...
    if (typeof observerOrNext === 'function') {
      return (this.asObservable() as Observable<T>).subscribe(observerOrNext, error, complete)
  ...
  pipe(...operations: UnaryFunction<Observable<T>, Observable<T>>[]): Observable<T> {
