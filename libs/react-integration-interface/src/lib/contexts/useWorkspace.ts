...
const useWorkspace = () => {
  ...
  const getUrl = (
    ...
    return currentWorkspace$.pipe(
      map((workspace: Workspace) =>
        constructRouteUrl(workspace, appId, productName, endpointName, endpointParameters, logger.warn)
      )
    )
  }
  ...
  const doesUrlExistFor = (productName: string, appId: string, endpointName?: string): Observable<boolean> => {
    return currentWorkspace$.pipe(
      map((workspace: Workspace) => doesUrlExistForWorkspace(workspace, productName, appId, endpointName, logger.warn))
    )
  }
