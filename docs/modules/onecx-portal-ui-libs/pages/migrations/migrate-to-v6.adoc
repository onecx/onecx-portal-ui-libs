...
=>  *Use standalone mode xref:documentation:docs-guides-ui:angular/standalone.adoc[Documentation]*
...
bootstrapRemoteComponent(RemoteComponent, 'ocx-my-remote-component', environment.production, [
  ...
  provideTranslationPathFromMeta(import.meta.url, 'assets/i18n/feature/')
]);
```
[NOTE]
====
...
[TIP]
====
Please use `createTranslateLoader` from `@onecx/angular-utils` in your `bootstrap.ts` instead of a custom `TranslateLoader`, as this supports multiple translation paths. Please consider using `MultiLanguageMissingTranslationHandler` to handle missing translation keys in your remote components, as it provides logging, fallback, and easy diagnostics for missing keys.
====
