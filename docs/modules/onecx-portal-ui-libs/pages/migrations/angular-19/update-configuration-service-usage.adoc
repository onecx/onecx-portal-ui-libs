...
In OneCX v6, three `ConfigurationService` methods have been updated to be asynchronous, returning `Promise` results instead of synchronous values.

**Update following methods only when ConfigurationService is imported from `@onecx/angular-integration-interface`**:
...
* Outside observables: use `await` in an `async` fn; if `async` not allowed (e.g. `constructor`, `ngOnInit`), resolve the Promise with `.then(...)`.
