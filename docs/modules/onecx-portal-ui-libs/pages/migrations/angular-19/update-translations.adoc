...
[TIP]
====
If you need to support multiple locales and fallback for missing translations, use `AngularAcceleratorMissingTranslationHandler` from `@onecx/angular-utils` as the `MissingTranslationHandler`. This handler logs missing translation keys and provides a fallback mechanism. For a reference see the example in the <<configure-remote-component-translation-providers>> section.
====
