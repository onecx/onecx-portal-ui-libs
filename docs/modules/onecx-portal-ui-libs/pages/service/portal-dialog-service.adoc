...
NOTE: Please, make sure that providePortalDialogService is used in in the *remote module's providers array* or in *remote component's providers array* (in case of standalone components usage). That way it will be accessible for all feature modules of the application.
...
Allowed severity values (matching PrimeNG's documented button severities):
* `primary`
* `secondary`
* `success`
* `info`
