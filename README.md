...
`merged_migrations='{"migrations": []}'; packages=(); merge_migrations() { [ -f "migrations.json" ] && merged_migrations=$(jq -s '.[0].migrations + .[1].migrations | unique_by(.name) | {migrations: .}' <(echo "$merged_migrations") migrations.json) && rm migrations.json; }; for dir in node_modules/@onecx/*; do [ -d "$dir" ] && packages+=("@onecx/$(basename "$dir")"); done; dependencies=$(jq -r '.dependencies | keys[] | select(startswith("@onecx/"))' package.json); devDependencies=$(jq -r '.devDependencies | keys[] | select(startswith("@onecx/"))' package.json); for dep in $dependencies $devDependencies; do [[ ! " ${packages[@]} " =~ " ${dep} " ]] && packages+=("$dep"); done; for package in "${packages[@]}"; do npx nx migrate "$package"; merge_migrations; done; echo "$merged_migrations" > migrations.json; if ! npx nx migrate --run-migrations --if-exists; then rm -rf node_modules package-lock.json; npm install --package-lock-only; npx nx migrate --run-migrations --if-exists; npm install --package-lock-only; fi; npm install`
